    ...
    def __init__(
        ...
        if rayd is not None:
            ...
            pix_grid = create_meshgrid(
                self.image_height, self.image_width, normalized_coordinates=False, device="cpu"
            )[0]
            ...
            ndc_camera = torch.cat((ndc_x, ndc_y, torch.ones_like(ndc_y) * (1.0), torch.ones_like(ndc_y)), 2)  # N,4
...
class MiniCam:
    def __init__(self, width, height, fov_y, fov_x, z_near, z_far, world_view_transform, full_proj_transform):
...
class Camerass(nn.Module):
    def __init__(
        ...
        if rayd is not None:
            ...
            projected = ndc_camera @ project_inverse.T
            direction_in_local = projected / projected[:, :, 3:]  #
