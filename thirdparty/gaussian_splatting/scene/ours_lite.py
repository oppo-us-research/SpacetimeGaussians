...
class GaussianModel:
    ...
    def load_ply_and_min_max_all(self, path, max_x, max_y, max_z, min_x, min_y, min_z):
        ...
        mask = np.logical_or(mask, mask_st)  # only use large tscale points.
    ...
    def add_gaussians(
        ...
        feature_dc = rgbs  # torch.cat((rgbs, torch.zeros_like(rgbs)), dim=1)# should we add the feature dc with non zero values?
        ...
        for z_scale in ray_list:
            ...
            ndc_camera = torch.cat(
                (ndc_v, ndc_u, torch.ones_like(ndc_u) * (1.0), torch.ones_like(ndc_u)), 1
            )  # N,4 ...
            ...
            world_point_H = local_point @ camera2wold.T  # myproduct4x4batch(local_point, camera2wold) #
            ...
            # new_scaling.append(newscalingmean.repeat(N_points,1))
            # new_rotation.append(newrotationmean.repeat(N_points,1))
            new_features_dc.append(feature_dc.cuda(0)[selected_mask])
            # new_opacity.append(newopacitymean.repeat(N_points,1))
    ...
    def reducepointsopscaleby_mask(self, selected_pts_mask):
        # restrict orignal one
        meanvalue = torch.min(self.get_opacity)
        opacities_new = inverse_sigmoid(
            torch.min(
                torch.ones_like(self.get_opacity[selected_pts_mask]) * meanvalue,
